        ...
        <div class="col-12 col-md-8">
            <nav aria-label="breadcrumb" class="mt-2 bg-white p-3 mb-3 rounded">
                <ol class="breadcrumb m-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Search date from "<?php echo($_GET['start']) ?>" to "<?php echo($_GET['end']) ?>"</li>
                </ol>
            </nav>
            <?php if(count(searchPostByDate($_GET['start'],$_GET['end'])) == 0) {
                echo alert("There is no result.","danger");
            } ?>
            <?php foreach(searchPostByDate($_GET['start'],$_GET['end']) as $p) { ?>
